...
readonly VERSION="nightly-2019-11-28"
readonly FLAGS_BUGGY="-Zmir-opt-level=2 -Copt-level=1"
readonly FLAGS_CORRECT1="-Zmir-opt-level=1 -Copt-level=1"
readonly FLAGS_CORRECT2="-Zmir-opt-level=2 -Copt-level=0"
...
if grep "panicked" "${OUTPUT_CORRECT2}" >& /dev/null ; then
  exit 1
fi
